...
# ============================================
# MODELO AUDIT LOG - AUDIT TRAIL (v1.1)
# ============================================

class AuditLog(db.Model):
    """Registro de auditoría para tracking de cambios en el sistema"""
    ...
    # tablas: 'expediente', 'audiencia', 'documento', 'usuario'
    ...
    # ¿Qué campo específico? (para ediciones)
    campo = db.Column(db.String(50), nullable=True)
    
    # Valores antes y después (solo para ediciones)
    ...
    # ¿Desde qué IP? (para seguridad)
    ip_address = db.Column(db.String(45), nullable=True)
    
    # ¿Desde qué navegador/dispositivo?
    user_agent = db.Column(db.String(200), nullable=True)
    
    def __repr__(self):
        return f'<AuditLog {self.accion} {self.tabla} #{self.registro_id} por {self.usuario}>'
    
    def get_accion_icono(self):
        """Devuelve icono según tipo de acción"""
        iconos = {
            'crear': '➕',
            'editar': '✏️',
            'eliminar': '🗑️',
    ...
    def get_accion_color(self):
        """Devuelve color según tipo de acción"""
        colores = {
            'crear': 'success',
            'editar': 'warning',
            'eliminar': 'danger',
...
class BackupLog(db.Model):
    """Registro de backups generados"""
    __tablename__ = 'backup_logs'
    
    id = db.Column(db.Integer, primary_key=True)
    nombre_archivo = db.Column(db.String(255), nullable=False)
    ruta_archivo = db.Column(db.String(500), nullable=False)
    tipo = db.Column(db.String(20), default='manual')  # manual o automatico
    tamaño_bytes = db.Column(db.BigInteger)
    descripcion = db.Column(db.Text)
    fecha_creacion = db.Column(db.DateTime, default=datetime.utcnow)
    creado_por = db.Column(db.String(50))
    estado = db.Column(db.String(20), default='completado')  # completado, error, restaurado
    
    def __repr__(self):
        return f'<BackupLog {self.nombre_archivo}>'
    
    def tamaño_formateado(self):
        """Devuelve el tamaño en formato legible"""
        bytes_val = self.tamaño_bytes or 0
